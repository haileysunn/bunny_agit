...
- 마커 클릭으로 상세 정보 확인
- 내 위치로 이동 버튼
- 🏢 **공공데이터 연동** (서울시 7개 구 흡연구역 데이터 등록 완료)
...
### 🛠️ 관리자 기능
- CSV 파일 업로드를 통한 공공데이터 동기화
- 컬럼 매핑 UI (시설명, 주소, 위도, 경도, 실내외 구분)
- 주소 기반 자동 좌표 변환 (Kakao Geocoding API)
- 중복 데이터 자동 감지 및 날짜 업데이트
- 진행상황 프로그레스바
- 관리자 권한 체크 (이메일 기반)
...
### External APIs
- **Map**: Kakao Maps API
- **Geocoding**: Kakao Geocoding API (주소 → 좌표 변환)
- **Location**: Geolocation API
- **Public Data**: 서울시 공공데이터 (광진구, 관악구, 노원구, 중구, 서대문구 등)
...
5. **🛠️ CSV 동기화** (`/admin/csv-sync`)
   - CSV 파일 업로드
   - 컬럼 매핑 (시설명, 주소, 위도, 경도 등)
   - 데이터 출처 및 등록일 설정
   - 자동 중복 체크 및 날짜 업데이트
